...
export default function WorkspaceMapPage() {
      ...
      <div className="flex-1 flex items-center justify-center bg-slate-50 dark:bg-slate-950">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-sm font-medium text-muted-foreground">
            {language === "hr" ? "Učitavanje interaktivne karte..." : "Loading interactive map..."}
          </span>
        </div>
      </div>
    );
  ...
  const defaultCenter: [number, number] = [45.815, 15.9819];
  ...
  const formatLastSeen = (lastSeen: string | null) => {
    if (!lastSeen) return language === "hr" ? "Nikada" : "Never";
    const diff = Date.now() - new Date(lastSeen).getTime();
    const mins = Math.floor(diff / 60000);
    const hours = Math.floor(mins / 60);
    const days = Math.floor(hours / 24);
    if (days > 0) return language === "hr" ? `prije ${days}d` : `${days}d ago`;
    if (hours > 0) return language === "hr" ? `prije ${hours}h` : `${hours}h ago`;
    if (mins > 0) return language === "hr" ? `prije ${mins}min` : `${mins}m ago`;
    return language === "hr" ? "upravo sada" : "just now";
  };

  return (
    <div className="flex flex-col lg:flex-row h-[calc(100vh-100px)] -m-6 overflow-hidden bg-slate-50 dark:bg-slate-950 font-sans">
      {/* ── Sidebar ── */}
      <div className="w-full lg:w-[380px] border-b lg:border-b-0 lg:border-r border-slate-200 dark:border-slate-800 bg-white/70 dark:bg-slate-900/80 backdrop-blur-md flex flex-col z-20 shrink-0 h-[45%] lg:h-full">
        {/* Header */}
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {locatedDevices.length}{" "}
                {language === "hr" ? "lociranih uređaja" : "devices located"}
              </p>
            </div>
            <button
              onClick={() => refetch()}
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-8 text-center p-4">
              ...
              <p className="text-xs text-muted-foreground/70 mt-1 max-w-[200px]">
                {language === "hr"
                  ? "Podesite filtre ili provjerite koordinate uređaja."
                  : "Adjust your filters or ensure devices have coordinates configured."}
              </p>
            </div>
          ) : (
      ...
      <div className="flex-1 relative h-[55%] lg:h-full z-10">
        <Map
          ...
          onBoundsChanged={({ center, zoom }) => {
            ...
            if (isProgrammaticRef.current) {
              if (
                Math.abs(center[0] - currentCenter[0]) < 0.001 &&
        ...
        >
          ...
          {filteredDevices.map((device) => {
            ...
            return (
              <Marker key={device.id} anchor={[device.latitude!, device.longitude!]}>
                <button
                  onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                  ...
                  onMouseEnter={() => !selectedDevice && setHoveredDeviceId(device.id)}
                  onMouseLeave={() => !selectedDevice && setHoveredDeviceId(null)}
                ...
                >
                  {/* Pulse ring */}
                  {isOnline && (
                    <div className="absolute inset-0 m-2 bg-indigo-500 rounded-full animate-ping opacity-30 pointer-events-none" />
                  )}
                  ...
                  <div
                    className={cn(
                      ...
                      (isHovered || isSelected) && "scale-125 ring-4 ring-indigo-500/40"
                    )}
                  >
                    <Radio className="h-3 w-3" />
                  </div>

                  {/* Hover Tooltip (shown only when no device selected) */}
                  {isHovered && !selectedDevice && (
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 px-4 py-3 bg-[#1e2538] text-white rounded-2xl shadow-2xl border border-slate-800 flex flex-col items-center gap-2.5 pointer-events-none animate-in fade-in zoom-in-95 duration-200 z-[200] min-w-[190px]">
                      <div className="flex items-center justify-between w-full gap-3">
                        <div className="flex-1 flex flex-col items-center text-center">
                          <span className="font-bold text-xs tracking-tight text-white leading-normal max-w-[140px] break-words">
                            {device.name}
                          </span>
                        ...
                        <div
                          className={cn(
                            "w-1.5 h-5.5 rounded-full shrink-0",
                            isOnline
                      ...
                      <span
                        className={cn(
                          "px-3 py-1 rounded-full text-[9px] font-extrabold tracking-wider border",
                          isOnline
                      ...
                      >
                        {isOnline
                          ? language === "hr" ? "NA MREŽI" : "ONLINE"
                          : language === "hr" ? "IZVAN MREŽE" : "OFFLINE"}
                      </span>
                      <div className="text-[10px] text-slate-400 font-mono font-medium">
                        {device.latitude?.toFixed(6)}, {device.longitude?.toFixed(6)}
                      </div>
                      <div className="absolute top-full left-1/2 -translate-x-1/2 border-[6px] border-transparent border-t-[#1e2538]" />
                    </div>
                  )}
                </button>
              </Marker>
        ...
        {/* ── Click Popup Overlay (rendered OUTSIDE <Map> to avoid clipping) ── */}
        {selectedDevice && (() => {
          const isOnline = selectedDevice.status === "online";
          return (
            <div
              className="absolute top-4 right-4 z-[500] animate-in fade-in slide-in-from-top-2 duration-200"
              style={{ minWidth: "240px", maxWidth: "280px" }}
              onClick={(e) => e.stopPropagation()}
            >
              <div className="bg-white dark:bg-[#1a2035] rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                {/* Header */}
                <div
                  className={cn(
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    {selectedDevice.serial_number && (
                      <p className="text-[10px] text-slate-400 font-semibold ml-4">
                        {selectedDevice.serial_number}
                      </p>
                    )}
                  </div>
                  <button
                    onClick={handleClosePopup}
                    className="shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
                  >
                ...
                <div className="px-4 py-3 space-y-2.5">
                  {/* Status */}
                  <div className="flex items-center gap-2.5 py-1.5 px-3 rounded-xl bg-slate-50 dark:bg-slate-800/50">
                    {isOnline ? (
                  ...
                  {selectedDevice.last_seen && (
                    <div className="flex items-center gap-2 text-[11px] text-slate-500 dark:text-slate-400">
                      ...
                      <span>
                        {language === "hr" ? "Zadnja aktivnost:" : "Last seen:"}{" "}
                  ...
                  {/* Coordinates */}
                  <div className="flex items-center gap-2 text-[11px] text-slate-500 dark:text-slate-400">
                    <Navigation className="h-3.5 w-3.5 shrink-0 text-slate-400" />
                  ...
                  {/* EUI */}
                  {selectedDevice.dev_eui && (
                    <div className="text-[10px] text-slate-400 dark:text-slate-500 font-mono bg-slate-50 dark:bg-slate-800/50 rounded-lg px-2.5 py-2 tracking-wider">
                      EUI: {selectedDevice.dev_eui.toUpperCase()}
                    </div>
                  )}
                </div>
                ...
                <div className="px-4 pb-4">
                  <button
                    onClick={() => router.push(`/${workspaceId}/devices/${selectedDevice.id}`)}
                    className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white text-[12px] font-bold tracking-wide transition-all duration-150 shadow-md shadow-indigo-500/20"
                  >
                    <ExternalLink className="h-3.5 w-3.5" />
                    {language === "hr" ? "Detalji uređaja" : "Device Details"}
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="absolute bottom-4 left-4 z-20 pointer-events-none">
          <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-md px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-800 text-[10px] font-bold text-indigo-600 dark:text-indigo-400 shadow-sm flex items-center gap-1.5">
            ...
            {language === "hr" ? "VIEWPORT SINKRONIZIRAN" : "MAP VIEWPORT SYNCHRONIZED"}
          </div>
        </div>
      </div>
    </div>
